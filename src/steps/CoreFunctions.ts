...
Given('the user navigates to {string}', async function (this: CustomWorld, url: string) {
    await this.page.goto(url, { waitUntil: 'domcontentloaded', timeout: 35000 }); // timeout to wait for DOM to be loaded.
...
When('the user types on element found by {string} {string} the text {string}', async function (this: CustomWorld, locatorType: string, locatorValue: string, textValue: string) {
    let locator;
    const clickTimeout = 15000; // Timeout by default for clicking actions

    console.log(`[Step] Attempting to type on element found by "${locatorType}" "${locatorValue}"`);
    ...
    console.log(`[Step] Successfully typed on element found by "${locatorType}" "${locatorValue}"`);
});
